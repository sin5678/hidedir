...
PVOID getDirEntryFileName(IN PVOID FileInformation,IN FILE_INFORMATION_CLASS FileInfoClass)
{
	PVOID result = 0;
...
ULONG getDirEntryFileNameLength(IN PVOID FileInformation, IN FILE_INFORMATION_CLASS FileInfoClass)
{
	ULONG result = 0;
	switch (FileInfoClass){
	case FileDirectoryInformation:
...
ULONG getNextEntryOffset(IN PVOID FileInformation,IN FILE_INFORMATION_CLASS FileInfoClass)
{
	ULONG result = 0;
	switch (FileInfoClass){
	case FileDirectoryInformation:
...
void setNextEntryOffset(IN PVOID FileInformation, IN FILE_INFORMATION_CLASS FileInfoClass, IN ULONG newValue)
{
	switch (FileInfoClass){
	case FileDirectoryInformation:
	...
	IN BOOLEAN RestartScan)
{
	if (av_CheckHookState())
	{
		...
		if (STATUS_SUCCESS == status)
		{
			...
			{
				PVOID pPrev = NULL;
				PVOID pFileInfo = (PVOID )FileInformation;
					...
					else
					{
						__try
						{
							do
							{
								...
								bLastFlag = !(getNextEntryOffset(pFileInfo,FileInformationClass));
								...
								RtlCopyMemory(wFileName, 
									getDirEntryFileName(pFileInfo, FileInformationClass), 
									getDirEntryFileNameLength(pFileInfo,FileInformationClass));
								...
								if (av_IsFileNameInHideList(FullFileName)) //
								{
									...
									{
										if (pPrev)
											setNextEntryOffset(pPrev,FileInformationClass,  0);
										else
										{
									...
									else
									{
										if (pPrev)
										{
											setNextEntryOffset(pPrev, 
												FileInformationClass,
												getNextEntryOffset(pPrev, FileInformationClass) + 
												getNextEntryOffset(pFileInfo, FileInformationClass));
											//pPrev->NextEntryOffset += pFileInfo->NextEntryOffset;
											pFileInfo =
												(PVOID)((CHAR*)pFileInfo +  getNextEntryOffset(pFileInfo,FileInformationClass));
										}
										else
										{
											//第一个文件就是啊 ..
											...
											RtlCopyMemory((PVOID)pFileInfo,
												(PVOID)((char *)pFileInfo + getNextEntryOffset(pFileInfo, FileInformationClass)),
												Length - getNextEntryOffset(pFileInfo, FileInformationClass));
										}
										continue;
									}
								}
								pPrev = pFileInfo;
								pFileInfo =
									(FILE_BOTH_DIR_INFORMATION *)((CHAR*)pFileInfo + getNextEntryOffset(pFileInfo, FileInformationClass));
							} while (!bLastFlag);
						}
						__except (EXCEPTION_EXECUTE_HANDLER)
						{
